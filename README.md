...
`eskit` 是一个 **WSL 优先** 的 Windows 文件搜索命令行工具。
...
| 版本 | 语言 | 源码目录 |
|------|------|---------|
| eskit-python | Python | [eskit-python/](eskit-python/) |
| eskit-rust | Rust | [eskit-rust/](eskit-rust/) |
...
`eskit` 依赖以下两个 Windows 工具：

| 工具 | 作用 |
|---|---|
| Everything | Windows 端文件索引和搜索引擎 |
| es.exe | Everything 官方命令行接口 |
| eskit | 面向 WSL/命令行日常使用的语法包装、路径转换、排序、统计、选择和动作层 |
...
| 版本 | 推荐场景 | 安装难度 |
|------|---------|---------|
| **eskit-python** | 日常使用、AI agent 集成、跨平台 | ⭐ 简单（pip） |
| **eskit-rust** | 追求极致性能、嵌入式 / 静态二进制 | ⭐⭐ 中等（需编译） |
...
```bash
git clone https://github.com/deliuou/eskit-python.git
cd eskit-python
...
```bash
git clone https://github.com/deliuou/eskit-rust.git
cd eskit-rust
cargo build --release
sudo cp target/release/eskit /usr/local/bin/eskit
eskit doctor
```
...
两个版本都会自动把 WSL 路径、盘符简写和 Windows 路径统一成 Everything 能理解的 Windows 路径。
...
```
eskit/
├── README.md           ← 统一说明文档（本文件）
├── eskit-python/       ← Python 版本
│   ├── README.md       ← Python 版独立文档
│   ├── pyproject.toml
│   ├── eskit/          ← 源码
│   └── ...
└── eskit-rust/        ← Rust 版本
    ├── README.md       ← Rust 版独立文档
    ├── Cargo.toml
    ├── src/
    └── ...
```
