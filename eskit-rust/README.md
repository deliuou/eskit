...
> ⚠️ **统一文档已移至 [父目录 README.md](../README.md)**，包含两个版本的完整安装和使用说明。本文件仅保留 Rust 版本的发布相关说明。

`eskit-rust` 是一个 **WSL 优先** 的 Windows 文件搜索命令行工具。
...
`eskit-rust` 依赖以下两个 Windows 工具：

| 工具 | 作用 |
|---|---|
| Everything | Windows 端文件索引和搜索引擎 |
| es.exe | Everything 官方命令行接口 |
| eskit-rust | 面向 WSL/命令行日常使用的语法包装、路径转换、排序、统计、选择和动作层 |
...
`eskit-rust` 会自动把 WSL 路径、盘符简写和 Windows 路径统一成 Everything 能理解的 Windows 路径。
